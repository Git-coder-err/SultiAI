...
export default function AdminLoginPage() {
  ...
  async function handleGoogle() {
    ...
    toast.push("success", "Signed in as Genesis Diaz (admin).");
    router.push("/admin");
  }
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    setBusy(true);
    await sessionMock.signIn();
    toast.push("success", "Signed in. Welcome back!");
    router.push("/admin");
  }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-brand-light to-surface px-4">
      <div className="w-full max-w-md rounded-3xl border border-line bg-white p-8 shadow-sm">
        ...
        <form onSubmit={handleEmail} className="mt-8 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-line bg-white px-4 py-3 text-sm text-ink outline-none transition-colors placeholder:text-ink-faint focus:border-brand"
            />
          </div>
          <div>
          ...
          <button
            type="submit"
            disabled={busy}
            className="w-full rounded-xl bg-gradient-to-r from-brand to-brand-dark py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-60"
          >
        ...
        <button
          ...
          disabled={busy}
          className="flex w-full items-center justify-center gap-3 rounded-xl border border-line bg-white py-3 text-sm font-semibold text-ink transition-colors hover:border-brand disabled:opacity-60"
        >
        ...
        <p className="mt-6 text-center text-xs leading-relaxed text-ink-faint">
          Mock sign-in for development. Backend auth will replace this when the API ships.
        </p>
      </div>
    </div>
  );
